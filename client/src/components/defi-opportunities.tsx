  ...
  return (
    <section className="px-4 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-primary">DeFi Opportunities</h2>
        <div className="flex items-center space-x-1">
          <Info className="h-4 w-4 text-neutral" />
          <span className="text-xs text-neutral">Learn more</span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {protocols?.map((protocol) => {
          ...
          return (
            <div key={protocol.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-primary">{protocol.name} {protocol.poolName}</h3>
                    <div className="flex items-center space-x-2">
                      <Badge variant="secondary" className={getRiskColor(protocol.riskLevel)}>
                        {protocol.riskLevel} risk
                      </Badge>
                      <span className="text-xs text-neutral">• {protocol.network}</span>
                    </div>
                  </div>
                </div>
              ...
              {isHighRisk && (
                <Alert className="mb-3 border-yellow-200 bg-yellow-50">
                  <AlertDescription className="text-xs text-yellow-800 flex items-center">
